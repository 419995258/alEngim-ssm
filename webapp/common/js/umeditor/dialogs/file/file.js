...
//                }
            ...
            callback: function (editor, $w, url, state) {
                ...
                if (state == "SUCCESS") {
                    //显示图片计数+1
                    Upload.showCount++;
                    var $img = $("<img src='" + editor.options.filePath + url + "' class='edui-image-pic' />"),
                ...
                me.dialog.find('.edui-image-content').on('drop',function (e) {
                    ...
                    $.each(fileList, function (i, f) {
                        if (/^image/.test(f.type)) {
                            ...
                            xhr.open("post", me.editor.getOpt('fileUrl') + "?type=ajax", true);
                            xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");

                            //模拟数据
                            var fd = new FormData();
                            fd.append(me.editor.getOpt('fileFieldName'), f);
    ...
    UM.registerWidget('file', {
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=file_url%>file.css\">" +
        ...
        initContent: function (editor, $dialog) {
            var lang = editor.getLang('image')["static"],
                opt = $.extend({}, lang, {
                    file_url: UMEDITOR_CONFIG.UMEDITOR_HOME_URL + 'dialogs/file/'
                });
