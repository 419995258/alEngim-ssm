                ...
                success : (data) => {
                    ...
                    if(totalPage <= 5){
                        ...
                        if(currentPage + 5 <= totalPage){
                            for(var i=1;i<6;i++){
                                htmladdStr+="<a href=\"javascript:;\" data-page='" + (currentPage + i) + "'>"+ (currentPage + i) +"</a>"
                            }
                            ...
                            var isnowPage = '';
                            for(var i=5;i>0;i--){
                                if(totalPage - i == currentPage){
                                    htmladdStr+="<a href=\"javascript:;\" data-page='" + (totalPage - i) + "' class=\"now-page\">"+ (totalPage - i) +"</a>"
                                }else{
                                    htmladdStr+="<a href=\"javascript:;\" data-page='" + (totalPage - i) + "'>"+ (totalPage - i) +"</a>"
                                }
                                if(totalPage  == currentPage){
                                    isnowPage = isnowPage + 'class=\'now-page\'';
                                }
                            }
                            htmlStr+="<a href=\"javascript:;\"  class=\"pre-page\" data-page='" + (currentPage - 1) + "'>上一页</a>"+
                               /* "<a href=\"javascript:;\"   data-page='" + (currentPage - 1) + "'>" + (currentPage - 1) + "</a>"+*/
                                     htmladdStr +
                                // "<a href=\"javascript:;\" class=\"now-page\" data-page='" + (currentPage) + "'>"+ currentPage +"</a>"+
                                "<a href=\"javascript:;\" data-page='" + (totalPage) + "' " + isnowPage + ">"+ (totalPage) +"</a>"+
                                "<a href=\"javascript:;\" data-page='" + (currentPage + 1) + "' class=\"next-page\">下一页</a>"+
                                "<span>共"+ totalPage +" 页,到第</span>"+
                                "<input class=\"pages-in\"  id=\"searchInput\" type=\"number\"/>"+
                                "<span>页</span>"+
