...
public class FileBean {
	private static Logger log = LoggerFactory.getLogger(FileBean.class);
	...
	public static boolean makeDir(String _sDir, boolean _bCreateParentDir) {
		...
		if (_bCreateParentDir) {
			bReturn = file.mkdirs(); // 如果父目录不存在，则创建所有必需的父目录
		} else {
			bReturn = file.mkdir(); // 如果父目录不存在，不做处理
		}
	...
	public static boolean copyFile(String _sSrcFile, String _sDstFile)
		...
		try {
			String[] commands = new String[] { "cp", formatPath(_sSrcFile),
			...
			if (osName.startsWith("windows")) {
				if (osName.indexOf("windows 9") > -1) {
					commands = new String[] { "command.com", "/c", "copy",
							"/Y", "\"" + _sSrcFile + "\"",
							"\"" + _sDstFile + "\"" };
				} else {
					commands = new String[] { "cmd.exe", "/c", "copy", "/Y",
							"\"" + _sSrcFile + "\"", "\"" + _sDstFile + "\"" };
				}
			...
			e.printStackTrace();
			System.out.println("复制文件时发生异常:" + e);
			throw e;
